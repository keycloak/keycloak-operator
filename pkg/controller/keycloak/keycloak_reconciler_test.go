...
func TestKeycloakReconciler_Test_Creating_All(t *testing.T) {
	...
	//    4) Master Realm
	//    5) Database secret
	//    6) Postgresql Persistent Volume Claim
	//    7) Postgresql Deployment
	//    8) Postgresql Service
	//    9) Keycloak Service
	//    10) Keycloak Discovery Service
	...
	assert.IsType(t, model.KeycloakMasterRealm(cr), desiredState[4].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.DatabaseSecret(cr), desiredState[5].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.PostgresqlPersistentVolumeClaim(cr), desiredState[6].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.PostgresqlDeployment(cr, true), desiredState[7].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.PostgresqlService(cr, model.DatabaseSecret(cr), false), desiredState[8].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakService(cr), desiredState[9].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakDiscoveryService(cr), desiredState[10].(common.GenericCreateAction).Ref)
...
func TestKeycloakReconciler_Test_Updating_RHSSO(t *testing.T) {
	...
	currentState := &common.ClusterState{
		...
		KeycloakMasterRealm:             model.KeycloakMasterRealm(cr),
	}
...
func TestKeycloakReconciler_Test_Updating_All(t *testing.T) {
	...
	//    12) Keycloak Route
	assert.Equal(t, 13, len(desiredState))
	...
	assert.IsType(t, model.KeycloakMasterRealm(cr), desiredState[4].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.DatabaseSecret(cr), desiredState[5].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.PostgresqlPersistentVolumeClaim(cr), desiredState[6].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.PostgresqlDeployment(cr, true), desiredState[7].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.PostgresqlService(cr, model.DatabaseSecret(cr), false), desiredState[8].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.KeycloakService(cr), desiredState[9].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.KeycloakDiscoveryService(cr), desiredState[10].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.KeycloakDeployment(cr, model.DatabaseSecret(cr)), desiredState[11].(common.GenericUpdateAction).Ref)
	assert.IsType(t, model.KeycloakRoute(cr), desiredState[12].(common.GenericUpdateAction).Ref)
}
...
func TestKeycloakReconciler_Test_Creating_All_With_External_Database(t *testing.T) {
	...
	assert.IsType(t, model.KeycloakMasterRealm(cr), desiredState[1].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.PostgresqlService(cr, model.DatabaseSecret(cr), false), desiredState[2].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakService(cr), desiredState[3].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakDiscoveryService(cr), desiredState[4].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakProbes(cr), desiredState[5].(common.GenericCreateAction).Ref)
	assert.IsType(t, model.KeycloakDeployment(cr, model.DatabaseSecret(cr)), desiredState[6].(common.GenericCreateAction).Ref)
}
...
func TestKeycloakReconciler_Test_Should_Create_PDB(t *testing.T) {
	...
	// then
	assert.Equal(t, 11, len(desiredState))
	assert.IsType(t, common.GenericCreateAction{}, desiredState[10])
	assert.IsType(t, model.PodDisruptionBudget(cr), desiredState[10].(common.GenericCreateAction).Ref)
}
...
func TestKeycloakReconciler_Test_Should_Update_PDB(t *testing.T) {
	...
	assert.Equal(t, 11, len(desiredState))
	assert.IsType(t, common.GenericUpdateAction{}, desiredState[10])
	assert.IsType(t, model.PodDisruptionBudget(cr), desiredState[10].(common.GenericUpdateAction).Ref)
}
...
func TestKeycloakReconciler_Test_Setting_Resources(t *testing.T) {
	...
	postgresContainer := desiredState[7].(common.GenericCreateAction).Ref.(*v13.Deployment).Spec.Template.Spec.Containers[0]
...
func TestKeycloakReconciler_Test_Proxy_Settings(t *testing.T) {
	...
	envs := desiredState[9].(common.GenericCreateAction).Ref.(*v13.StatefulSet).Spec.Template.Spec.Containers[0].Env
	proxySet := false
...
func TestKeycloakReconciler_Test_Should_Create_Backup(t *testing.T) {
	...
	assert.Equal(t, 11, len(desiredState))
	assert.IsType(t, common.GenericUpdateAction{}, desiredState[10])
	assert.IsType(t, model.KeycloakMigrationOneTimeBackup(backupCr), desiredState[10].(common.GenericUpdateAction).Ref)
}
