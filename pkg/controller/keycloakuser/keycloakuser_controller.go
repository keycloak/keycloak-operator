...
func (r *ReconcileKeycloakUser) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	for _, realm := range realms.Items {
		...
		keycloakRealmRefCopied := realm.DeepCopy()

		if realm.Spec.Managed == kc.Unmanaged {
			return r.ManageError(instance, errors.Errorf("users cannot be created for unmanaged keycloak realms"))
		}
		if instance.Spec.Managed == kc.Managed || instance.Spec.Managed == "" {
			if keycloakRealmRefCopied.Spec.Managed == kc.Autogenerated {
				log.Info(fmt.Sprintf("Skipping an auto-generated Realm %v for the managed client %v/%v to avoid accidental damage", keycloakRealmRefCopied.Name, instance.Namespace, instance.Name))
				log.Info("Use different labels on Realms and Keycloak CRs to avoid this message")
				continue
			}
		}

		keycloaks, err := common.GetMatchingKeycloaks(r.context, r.client, keycloakRealmRefCopied.Spec.InstanceSelector)
		if err != nil {
		...
		for _, keycloak := range keycloaks.Items {
			...
			keycloakFactory := common.NewDefaultKeycloakConnectionFactory(r.context, r.client, keycloakRefCopied, keycloakRealmRefCopied)
