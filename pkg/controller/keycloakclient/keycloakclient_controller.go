...
func (r *ReconcileKeycloakClient) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if instance.Spec.Managed == kc.Unmanaged {
		return reconcile.Result{Requeue: false}, r.manageSuccess(instance, nil, instance.DeletionTimestamp != nil)
	}
	...
	var clientState *common.ClientState
	for _, realm := range realms.Items {
		...
		if realm.Spec.Managed == kc.Unmanaged {
			return r.ManageError(instance, errors.Errorf("clients cannot be created for unmanaged keycloak realms"))
		}
		...
		log.Info(fmt.Sprintf("found %v matching keycloak(s) for realm %v/%v", len(keycloaks.Items), keycloakRealmRefCopied.Namespace, keycloakRealmRefCopied.Name))
		...
		for _, keycloak := range keycloaks.Items {
			...
			log.Info(fmt.Sprintf("got authenticated client for keycloak for realm %v at %v", keycloakRealmRefCopied.Name, keycloak.Status.InternalURL))
	...
	return reconcile.Result{Requeue: false}, r.manageSuccess(instance, clientState, instance.DeletionTimestamp != nil)
}
...
func (r *ReconcileKeycloakClient) manageSuccess(client *kc.KeycloakClient, currentState *common.ClientState, deleted bool) error {
	...
	if currentState != nil && currentState.ClientSecret != nil {
		// Let the clients know where the admin credentials are stored
		client.Status.CredentialSecret = currentState.ClientSecret.Name
	}
