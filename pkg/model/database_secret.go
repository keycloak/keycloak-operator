...
func DatabaseSecret(cr *v1alpha1.Keycloak) *v1.Secret {
	return &v1.Secret{
		...
		StringData: map[string]string{
			DatabaseSecretUsernameProperty: cr.ObjectMeta.Name + "-" + GenerateRandomString(4),
			DatabaseSecretPasswordProperty: cr.ObjectMeta.Name + "-" + GenerateRandomString(4),
...
func DatabaseSecretReconciled(cr *v1alpha1.Keycloak, currentState *v1.Secret) *v1.Secret {
	...
	if _, ok := reconciled.Data[DatabaseSecretUsernameProperty]; !ok {
		reconciled.StringData[DatabaseSecretUsernameProperty] = cr.ObjectMeta.Name + "-" + GenerateRandomString(4)
	}
	if _, ok := reconciled.Data[DatabaseSecretPasswordProperty]; !ok {
		reconciled.StringData[DatabaseSecretPasswordProperty] = cr.ObjectMeta.Name + "-" + GenerateRandomString(4)
	}
