		...
		Spec: monitoringv1.PrometheusRuleSpec{
			Groups: []monitoringv1.RuleGroup{{
				...
				Rules: []monitoringv1.Rule{{
					...
					Annotations: map[string]string{
						"message": `Less than 90% of the RH SSO API endpoints in namespace {{ $labels.namespace }} are taking longer than 1s for the last 5 minutes.`,
					},
					Expr: intstr.FromString(`(sum(rate(keycloak_request_duration_bucket{le="1000.0"}[5m])) by (job) / sum(rate(keycloak_request_duration_count[5m])) by (job)) < 0.90`),
					For:  "5m",
					Labels: map[string]string{
						"severity": "critical",
					},
					...
					Expr: intstr.FromString(`(sum(rate(keycloak_request_duration_bucket{le="10000.0"}[5m])) by (job) / sum(rate(keycloak_request_duration_count[5m])) by (job)) < 0.995`),
					For:  "5m",
					Labels: map[string]string{
						"severity": "critical",
					},
