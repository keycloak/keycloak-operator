...
func TestKeycloakReconciler_Test_Creating_All(t *testing.T) {
	...
	assert.IsType(t, model.KeycloakDeployment(cr, model.DatabaseSecret(cr)), desiredState[10].(common.GenericCreateAction).Ref)
...
func TestKeycloakReconciler_Test_Creating_All_With_External_Database(t *testing.T) {
	...
	assert.IsType(t, model.KeycloakDeployment(cr, model.DatabaseSecret(cr)), desiredState[4].(common.GenericCreateAction).Ref)
}
